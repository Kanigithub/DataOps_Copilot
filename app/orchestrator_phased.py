...
from .artifact_writer import (
    write_text, write_notebook_from_md, write_sql_files_from_md,
...
- Deployment must include Databricks Asset Bundles + GitHub Actions.
...
def run_agents(
    ...
    # Load/restore summary between phases
    summary_path = run_dir / "memory_summary.txt"
    ...
    for agent in agents:
        ...
        artifacts.append(write_text(run_dir / f"{agent}.md", content))

        # update summary for next agent/phase
        memory_summary = summarize(content)
        summary_path.write_text(memory_summary, encoding="utf-8")

        # generate artifacts only in phase2 when those agents run
        ...
        if agent == "deployment":
            ...
            artifacts.append(write_github_actions_bundle_workflow(ddir / ".github" / "workflows" / "databricks-bundle.yml"))
